...
이후 비슷한 방법으로 Laycast를 이용 해 마법을 사용하는것을 구현했고
플레이어가 아이템을 던지는것을 구현중에 있습니다.
```C#
...
각 플레이어와 몬스터는 이동 전 주변 오브젝트를 판정해 이동가능/불가능을 판별해 이동합니다.

플레이어의 경우 간단하게 만들 수 있었지만
몬스터의 경우 A* 알고리즘을 짤 수 없었기 때문에 강사님이 짜주신것으로 대체했으나 학습중입니다.
완료하는대로 교체 후 업데이트 
